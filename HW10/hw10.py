...
def ifgsm(model, x, y, loss_fn, epsilon=epsilon, alpha=alpha, num_iter=20):
    ...
    for i in range(num_iter):
        ...
        #   model.eval()
        # if i % 5 == 4:
        #   model = ptcv_get_model('resnet272bn_cifar10', pretrained=True).to(device)
        #   model.eval()
...
class EnsembleModel(nn.Module):
  ...
  def forward(self, x):
    return ( self.model_1(x) + self.model_2(x) + self.model_3(x) + self.model_4(x) + self.model_5(x) ) / 5

model_1 = ptcv_get_model('resnet110_cifar10', pretrained=True).to(device)
model_2 = ptcv_get_model('preresnet1202_cifar10', pretrained=True).to(device)
model_3 = ptcv_get_model('resnext272_2x32d_cifar10', pretrained=True).to(device)
model_4 = ptcv_get_model('wrn40_8_cifar10', pretrained=True).to(device)
model_5 = ptcv_get_model('pyramidnet272_a200_bn_cifar10', pretrained=True).to(device)

model = EnsembleModel(model_1, model_2, model_3, model_4, model_5).to(device)
loss_fn = nn.CrossEntropyLoss()
